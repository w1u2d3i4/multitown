...
本分支现已加入上述发现矩阵的冻结后继方案 **MT-CapacityRoute-v1**。它保留
Planner→Executor 职责分离，只在三个由开发集预先选定的公开任务类别上启用强
Executor，其余任务使用经济型 Executor。它是确定性非 RL 方法。
...
## 当前 TeamBench 结果：容量感知角色分配

在用 generator seed 5 新生成的 89 个测试实例上，三种方法使用完全相同的任务实例
哈希、采样种子、模型端点、容器镜像、温度和输出上限：

| 策略 | 完全通过 | 平均部分分 | 平均 Token/任务 | p95 延迟 | 能耗 |
| --- | ---: | ---: | ---: | ---: | ---: |
...
MT-CapacityRoute 是**当前同一套本地协议中最强的工作点**：完全通过数和平均部分分
最高，同时平均 Token 最少。相对 Solo，它新增 6 个通过且没有丢失任何 Solo 已通过
任务（双侧精确 McNemar p=0.03125），Token 减少 42.56%，监控能耗减少 30.37%。
相对 PlanExecute，它净增 4 个通过，部分分提高 +0.02577，Token 减少 2.59%，能耗
减少 6.74%。

结论边界同样重要：相对两个基线的部分分配对区间都跨 0；75 个未改变弱路由的任务
仍存在本地推理波动。单独审计的 14 个真正改变执行器容量的任务方向为正（部分分
+0.11096，6 胜/7 平/1 负），基线锚定估计为 18 个通过、0.63348 部分分。因此这里
声称的是**同协议点估计胜出**，不是部分分统计显著优越，更不是跨论文 SOTA。详见
[正式记录](public_bench/records/TEAMBENCH_CAPACITY_ROUTE_TEST_V1.md)和
[冻结协议](public_bench/docs/TEAMBENCH_CAPACITY_ROUTE_V1_PROTOCOL.md)。

## 历史 TeamBench 主流策略矩阵（seed 0）
...
因此这组发现证据不是简单的“多 Agent 更好”，而是：**规划与交接可以很高效；没有
修复闭环时，盲目增加角色反而可能更差。** 原始 MultiTown-TB 动态控制器并非五种
策略中的赢家，它作为有价值的负结果，直接促成了上面的容量感知后继方案。详见
[正式对比记录](public_bench/records/TEAMBENCH_STRATEGY_QUALITY_V2.md)
...
MT-Agentic-RL-v3 进一步加入 p90 尾部成本储备和请求级预算守卫。在 seed 4 上，
它把通过数从 5/30 提高到 6/30，平均部分分从 0.64510 提高到 0.67667，且没有
任务超过 90k Token；但平均 Token 增幅为 20.223%，比冻结的 20% 上限高
0.223 个百分点，质量区间下界仍为 0。实验后没有放宽阈值：这是目前最强的
